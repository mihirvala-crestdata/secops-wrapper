...
def test_cli_parser_lifecycle(cli_env, common_args):
    ...
    try:
        ...
        create_cmd = [
                         ...
                         "parser", "create",
                         "--log-type", test_log_type, # Positional argument for log_type
        ...
        try:
            ...
            pytest.fail(f"Could not parse JSON from create command output: {create_result.stdout}")
        ...
        if created_parser_id:
            ...
            if clean_up_result.returncode != 0:
                print(f"WARNING: Cleanup deletion failed for {created_parser_id}: "
                      f"{clean_up_result.stderr}\n{clean_up_result.stdout}")

        # Clean up temporary parser code file
        if 'parser_code_file_path' in locals() and os.path.exists(parser_code_file_path):
